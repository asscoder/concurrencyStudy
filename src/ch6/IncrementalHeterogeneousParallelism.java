...
public class IncrementalHeterogeneousParallelism {

	private final String MATCHER_PATTERN = "\\bRandom[0-9]*\\b";
	private final ExecutorService pool = Executors.newFixedThreadPool(5);
	private final CompletionService<String> completionService = new ExecutorCompletionService<>(pool);
	...
	public void renderPage(String inputString) throws MyError {
		...
		for (StringLocation location : locations) {
			completionService.submit(new StringRandomizerCallable(location));
		}
	...
	public static void main(String[] args) throws Exception {
		try {
			IncrementalHeterogeneousParallelism parallelism = new IncrementalHeterogeneousParallelism();
